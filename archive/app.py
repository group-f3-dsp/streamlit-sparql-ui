...
with st.sidebar:
    ...
    sparql_endpoint = st.text_input("SPARQL Endpoint", SPARQL_ENDPOINT)
    st.session_state['sparql_endpoint'] = sparql_endpoint

    api_key_input = st.text_input("Gemini API Key", api_key, type="password")
...
def convert_to_jsonld(df):
    ...
    for index, row in df.iterrows():
        for col in df.columns:
            s = URIRef(row[col]) if col == 'building' else URIRef(f"http://example.org/{col}")
            p = URIRef(f"http://example.org/{col}")
            o = Literal(row[col])
            graph.add((s, p, o))
...
LIMIT 10"""
    sparql_query = st.text_area("SPARQL Query", sparql_query, height=300, key="sparql_query_text_area_1")
